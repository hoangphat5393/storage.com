...
use App\Models\DangtinCheckDate;
use DB;
use Mail;
...
class PaypalTestController extends Controller
{
    ...
    public function testPost(Request $request)
    {
        ...
        if ($charge['object'] == 'charge') {
            ...
            $cart = \App\Models\Addtocard::find($respons->cart_id)->update(['cart_code' => $order_id]);
            ...
            foreach ($cart_content as $key => $value) {
                $data_addcard_detal = [
                    ...
                    'admin_id'      => \App\Models\Theme::find($value->id)->admin_id ?? 0,
                    'name'          => $value->title,
                    'quanlity'      => $value->qty,
                    'subtotal'      => $value->subtotal
                ];
                \App\Models\Addtocard_Detail::create($data_addcard_detal);
            }
    ...
    {
        $cart = \App\Models\Addtocard::where('cart_id', $cart_id)->first();
    ...
    public function payment_success(Request $request)
    {
        ...
        if ($request->input('paymentId') && $request->input('PayerID')) {
            ...
            if ($response->isSuccessful()) {
                ...
                $isPaymentExist = \App\Models\Addtocard::where('cart_id', $cart_id)->first();
                ...
                if ($isPaymentExist) {
                    ...
                    $content_success = \App\Models\Page::find(81);
    ...
    {
        if ($id) {
            $content_success = \App\Models\Page::find(83);
