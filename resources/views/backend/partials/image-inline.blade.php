<div class="group_item_theme">
    <div class="icon_change_postion"><i class="fa fa-sort"></i></div>
    <div class="left_item_theme left_genate">
        <input type="text" value="{{ $code_name ?? '' }}" placeholder="Please enter Name Field" name="header_option[img][name][]" />
    </div>
    <div class="right_item_theme right_genate">
        <div class="input-group">
            <div class="input-group">
                <input type="text" class="form-control input_image {{ $id ?? 'img' }}_view" name="header_option[img][value][]" id="{{ $name ?? 'image' }}" value="{{ $image ?? '' }}" autocomplete="off">
                <div class="input-group-append">
                    <button class="btn btn-outline-secondary ckfinder-popup" type="button" id="{{ $id ?? 'img' }}" data-show="{{ $id ?? 'img' }}_view" data="{{ $name ?? 'image' }}">Upload</button>
                </div>
            </div>
        </div>
    </div>
    <div class="action">
        <input type="button" class="btn btn-danger button button-secondary tbl_button_delete_clean" value="Delete" name="delete_tbl">
    </div>
</div>

{{--
<div class="row align-items-center mb-3">
    <div class="col-auto">
        <div class="icon_change_postion px-4">
            <i class="fa fa-sort"></i>
        </div>
    </div>
    <div class="col-10">
        <div class="row align-items-center">
            <div class="col-3">
                <div class="d-flex align-items-center">
                    <input type="text" value="{{ $code_name ?? '' }}" class="form-control" placeholder="Please enter Name Field" name="header_option[img][name][]" />
                </div>
            </div>
            <div class="col-8">
                <div class="input-group">
                    <div class="input-group">
                        <input type="text" class="form-control input_image {{ $id ?? 'img' }}_view" name="header_option[img][value][]" id="{{ $name ?? 'image' }}" value="{{ $image ?? '' }}" autocomplete="off">
                        <div class="input-group-append">
                            <button class="btn btn-outline-secondary ckfinder-popup" type="button" id="{{ $id ?? 'img' }}" data-show="{{ $id ?? 'img' }}_view" data="{{ $name ?? 'image' }}">Upload</button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-auto">
                <input type="button" class="btn btn-danger tbl_button_delete_clean" value="Delete" name="delete_tbl">
            </div>
        </div>
    </div>
</div> --}}
